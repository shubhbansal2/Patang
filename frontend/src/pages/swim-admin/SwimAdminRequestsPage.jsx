...
const SwimAdminRequestsPage = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      // Filter strictly for swimming
      const swimReqs = (data.data?.subscriptions || []).filter(r => r.facilityType?.toLowerCase() === 'swimming');
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">Swimming Subscription Requests</h1>
        <p className="text-sm text-gray-500 mt-1">Review and manage pool subscriptions.</p>
      </div>
                  ...
                  {request.status === 'Pending' && (
                    <>
                      <button
                        ...
                        className="flex items-center justify-center gap-2 px-6 py-2.5 bg-blue-500 text-white font-semibold rounded-xl hover:bg-blue-600 transition-colors shadow-sm"
                      >
                        <Check size={18} />
                        Approve
                      </button>
        ...
        <>
          ...
          {actionType === 'view_receipt' ? (
            ...
            <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl shadow-xl z-50 overflow-hidden">
              <div className={`p-6 border-b ${actionType === 'approve' ? 'border-blue-100 bg-blue-50' : 'border-red-100 bg-red-50'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <h3 className={`text-lg font-bold ${actionType === 'approve' ? 'text-blue-800' : 'text-red-800'}`}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all resize-none"
                  />
                </div>
...
export default SwimAdminRequestsPage;
