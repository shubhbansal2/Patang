...
const MetricTile = ({ title, value, icon: Icon, colorClass, linkTo }) => (
  <Link to={linkTo} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:border-blue-200 transition-colors block group">
...
const SlotManager = ({ facilityType }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 overflow-hidden mt-6">
                ...
                {editingSlot?._id === slot._id ? (
                  <form onSubmit={handleUpdateCapacity} className="flex items-center gap-2">
                    <input type="number" min="1" value={newCapacity} onChange={e => setNewCapacity(e.target.value)}
                      className="w-16 px-2 py-1 text-sm border border-gray-300 rounded-lg bg-white outline-none focus:border-blue-500" autoFocus />
                    <button type="submit" className="text-xs font-semibold text-white bg-blue-500 px-3 py-1.5 rounded-lg hover:bg-blue-600 transition-colors shadow-sm">Save</button>
                  ...
                  <button onClick={() => { setEditingSlot(slot); setNewCapacity(slot.capacity || ''); }}
...
const SwimAdminDashboardPage = () => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const res = await api.get('/v2/admin/subscriptions?status=Pending&limit=50');
        ...
        setData({
          ...
          totalPending: swimReqs.length,
        });
      } catch (err) {
        console.error('Swim Admin dashboard fetch error:', err);
        setError(err.response?.data?.message || 'Failed to load swimming dashboard');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col h-full">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/swim-admin/requests" className="text-xs font-semibold text-blue-500 hover:text-blue-600 transition-colors">
              View All
            </Link>
          </div>
...
export default SwimAdminDashboardPage;
