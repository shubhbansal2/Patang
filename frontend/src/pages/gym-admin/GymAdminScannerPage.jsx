...
const GymAdminScannerPage = () => {
  ...
  const [scanResult, setScanResult] = useState(null); // { success: true/false, data: {}, error: '' }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center sm:text-left">
        <h1 className="text-2xl font-bold text-gray-900">QR Access Scanner</h1>
        <p className="text-gray-500 mt-1">Scan or enter student access passes to verify gym and pool entry.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
          ...
          <div className="p-8 flex-1 flex flex-col justify-center">
            {/* Future visual QR scanner box could go here */}
            <div className="w-full aspect-square max-w-[240px] mx-auto bg-gray-50 border-2 border-dashed border-gray-300 rounded-3xl flex items-center justify-center mb-8 relative">
              <div className="absolute top-4 left-4 w-8 h-8 border-t-4 border-l-4 border-brand-500 rounded-tl-xl"></div>
              <div className="absolute top-4 right-4 w-8 h-8 border-t-4 border-r-4 border-brand-500 rounded-tr-xl"></div>
              <div className="absolute bottom-4 left-4 w-8 h-8 border-b-4 border-l-4 border-brand-500 rounded-bl-xl"></div>
              <div className="absolute bottom-4 right-4 w-8 h-8 border-b-4 border-r-4 border-brand-500 rounded-br-xl"></div>
            ...
            <form onSubmit={handleScan} className="w-full">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. GYM-12345"
                  className="w-full pl-11 pr-32 py-4 bg-gray-50 border border-gray-200 rounded-xl text-lg font-mono placeholder:font-sans focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all uppercase tracking-wider"
                  autoFocus
                />
                <button
                  type="submit"
                  disabled={loading || !passId.trim()}
                  className="absolute right-2 top-2 bottom-2 px-6 bg-brand-500 hover:bg-brand-600 disabled:opacity-50 disabled:hover:bg-brand-500 text-white font-bold rounded-lg transition-colors flex items-center justify-center min-w-[100px]"
                >
                  {loading ? (
              ...
              <div className="flex-1 flex flex-col animate-in fade-in zoom-in duration-300">
                ...
                <div className="space-y-4 flex-1">
                  ...
                  {scanResult.data.occupancy && (
                    <div className="flex items-center gap-3 p-3 bg-brand-50 border border-brand-100 rounded-xl mt-auto">
                      <ArrowRightLeft size={18} className="text-brand-500 shrink-0" />
                      <div>
